...
(function () {
  ...
  function getPathApi() {
    return window.vimixCameraPath || null;
  }
  ...
  function generatePathLog() {
    ...
    return [
      '// --- VIMIX CAMERA PATH START ---',
      JSON.stringify(path, null, 2),
      '',
      '// Runtime preview snippet:',
      `window.vimixCameraPath.setPath(${JSON.stringify(path)});`,
      '// --- VIMIX CAMERA PATH END ---'
    ].join('\n');
  }
  ...
  if (window.vimixEditorLogs) {
    window.vimixEditorLogs.camera = generateCameraLog;
    window.vimixEditorLogs.cameraPath = generatePathLog;
  }
  ...
  window.vimixCameraEditorRefresh = () => { if (isOpen) loadPathFrames(false); };
