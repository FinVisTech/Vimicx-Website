...
(function () {
  ...
  // ===== API accessor =====
  function api() { return window.vimixTextConfig || null; }
  ...
  // ===== Flicker bar helpers =====
  function flickerApi() { return window.vimixFlickerConfig || null; }
  ...
  function glassesApi() { return window.vimixGlassesRise || null; }
  ...
  function init() {
    ...
    window.vimixTextEditorRefresh = () => { items = api() ? api().getItems() : []; render(); };
    window.vimixFlickerEditorRefresh = render;
  ...
  const ready = setInterval(() => {
    if (window.vimixTextConfig && document.getElementById('camera-text-bars')) {
      clearInterval(ready);
      init();
    }
  }, 150);
