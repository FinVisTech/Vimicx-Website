...
(function () {
  ...
  function applyToScene() {
    ...
    // Lights
    if (typeof ambientLight !== 'undefined' && ambientLight) {
      if (inputs.ambColor.value) ambientLight.color.setHex(parseInt(inputs.ambColor.value.replace('#', '0x'), 16));
      ambientLight.intensity = parseFloat(inputs.ambInt.value);
    }
    if (typeof dirLight !== 'undefined' && dirLight) {
      if (inputs.dirColor.value) dirLight.color.setHex(parseInt(inputs.dirColor.value.replace('#', '0x'), 16));
      dirLight.intensity = parseFloat(inputs.dirInt.value);
    }
    if (typeof pointCyan !== 'undefined' && pointCyan) {
      if (inputs.cyanColor.value) pointCyan.color.setHex(parseInt(inputs.cyanColor.value.replace('#', '0x'), 16));
      pointCyan.intensity = parseFloat(inputs.cyanInt.value);
    }
    if (typeof pointMagenta !== 'undefined' && pointMagenta) {
      if (inputs.magColor.value) pointMagenta.color.setHex(parseInt(inputs.magColor.value.replace('#', '0x'), 16));
  ...
  function generateLog() {
    const snippet = 
      ...
      `\n` +
      `// Lights\n` +
      `ambientLight = new THREE.AmbientLight(${inputs.ambColor.value.replace('#', '0x')}, ${parseFloat(inputs.ambInt.value).toFixed(2)});\n` +
      `scene.add(ambientLight);\n` +
      `\n` +
      `dirLight = new THREE.DirectionalLight(${inputs.dirColor.value.replace('#', '0x')}, ${parseFloat(inputs.dirInt.value).toFixed(2)});\n` +
      `dirLight.position.set(5, 8, 5);\n` +
      `scene.add(dirLight);\n` +
      `\n` +
      `pointCyan = new THREE.PointLight(${inputs.cyanColor.value.replace('#', '0x')}, ${parseFloat(inputs.cyanInt.value).toFixed(2)}, 20);\n` +
      `pointCyan.position.set(-3, 4, 2);\n` +
      `scene.add(pointCyan);\n` +
      `\n` +
      `pointMagenta = new THREE.PointLight(${inputs.magColor.value.replace('#', '0x')}, ${parseFloat(inputs.magInt.value).toFixed(2)}, 15);\n` +
